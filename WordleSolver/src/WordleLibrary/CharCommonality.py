class CharCommonality:
    ...
    def GetLeastCommonLetters(
        self, words: list[str], inclDblAndTripleLetters: bool = False
    ) -> list[list[str]]:
        ...
        for charNVal in sortedList:
            ...
            if len(charNVal[0]) > 1:
                if (
                    self._GetCharCommonality(charNVal[0][0], charNVal[0][1] - 2)
                ) <= charNVal[1]:
                    continue
    ...
    def _AddCharCommonality(self, char: str, index: int):
        try:
            ...
            print(
                "\n\nERROR:\tCouldn't add commonality for a word with the same letter",
                index + 1,
                "times\n\n",
            )
